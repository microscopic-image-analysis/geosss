...
samplers['hmc'] = gs.SphericalHMC(target, np.copy(
    initial), stepsize=1e-3, n_steps=n_steps)
samplers['sss'] = (gs.RejectionSphericalSliceSampler(target, initial),
                   gs.ShrinkageSphericalSliceSampler(target, initial))[0]
...
fig, axes = plt.subplots(
    2, target.d//2, figsize=(10, 3), sharex=True, sharey=True)
for i, ax in enumerate(axes.flat):
    hx, bins = np.histogram(samples['Wood'][:, i], bins=30, density=True)
    ax.plot(0.5 * (bins[1:] + bins[:-1]), hx, lw=1, alpha=0.99, label='Wood',
            color=gs.colors[3])
...
for i, ax in enumerate(axes.flat):
    hx, bins = np.histogram(samples['Wood'][:, i], bins=30, density=True)
    ax.plot(0.5 * (bins[1:] + bins[:-1]), hx, lw=1, alpha=0.99,
            label='Wood', color=gs.colors[3])
...
fig, axes = plt.subplots(2, target.d//2, figsize=(8, 4),
