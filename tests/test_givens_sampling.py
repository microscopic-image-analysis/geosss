...
y = testing.metropolis_hastings(
    pdf, burnin + n_samples, stepsize=0.5, adapt_stepsize=not True
)[burnin::]

# Slice sampling using random Givens rotations and bracketing
z = testing.slice_sampling4(pdf, burnin + n_samples)[burnin:]
...
axes[2, 1].set_title("Spherical slice sampling")
...
axes[2, 0].set_title("Spherical slice sampling")
...
for i, (ax1, ax2, ax3, ax4) in enumerate(zip(*axes)):
    for ax, xx in zip([ax1, ax2, ax3, ax4], [x, y, z, w]):
        ax.plot(autocorr(xx[:, i])[:50], color="k", lw=3, alpha=0.7)
...
print(
    ("populations: {0:.1%} (Gibbs) {1:.1%} (MH) {2:.1%} (SSS) {3:.1%} (Kent)").format(
...
print(
    (
        "average hopping frequency: {0:.2e} (Gibbs) {1:.2e} (MH) "
        "{2:.1%} (SSS) {3:.1%} (Kent)"
    ).format(
        np.mean(np.fabs(np.diff(cx))),
        np.mean(np.fabs(np.diff(cy))),
        np.mean(np.fabs(np.diff(cz))),
        np.mean(np.fabs(np.diff(cw))),
    )
)
